...
| 3 | **TTS** | Planned | Convert narration to speech via ElevenLabs / LiveKit |
...
| Command | Description |
|---|---|
| `echo-copilot start [--port PORT] [--daemon] [--skip-hooks]` | Install hooks + start server |
...
| `GET` | `/health` | Server status, version, subscriber counts, Ollama status |
...
**271 tests** covering event types, event bus, hook handling, hook installation, transcript watching, server endpoints, narration types, template engine, event batching, LLM summarizer, core summarizer, and narration streaming.
