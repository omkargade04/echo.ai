"""Install and uninstall Echo hooks in Claude Code settings."""
...
from echo.config import CLAUDE_SETTINGS_PATH, HOOKS_DIR
...
# that lets us distinguish Echo hooks from user hooks.
_HOOK_COMMAND: str = "~/.echo-copilot/hooks/on_event.sh"
...
def install_hooks() -> None:
    """Install Echo hooks into Claude Code settings.

    1. Copy ``on_event.sh`` into ``~/.echo-copilot/hooks/``.
    2. Read ``~/.claude/settings.json`` (creating it if absent).
    3. Merge Echo hook entries, preserving existing user hooks.
    ...
    logger.info("Echo hooks installed successfully")


def uninstall_hooks() -> None:
    """Remove Echo hooks from Claude Code settings.

    Only removes hook entries whose ``command`` matches the Echo
    script path.  All other user hooks are preserved.  The
    ``~/.echo-copilot/hooks/`` directory is cleaned up afterwards.
    """
    ...
    logger.info("Echo hooks uninstalled successfully")


def are_hooks_installed() -> bool:
    """Return ``True`` if Echo hooks are present in Claude settings."""
    if not CLAUDE_SETTINGS_PATH.exists():
        return False
...
def _deploy_hook_script() -> None:
    """Copy the bundled ``on_event.sh`` to ``~/.echo-copilot/hooks/``."""
...
def _cleanup_hook_script() -> None:
    """Remove the deployed hook script from ``~/.echo-copilot/hooks/``."""
...
def _merge_hooks(settings: dict) -> None:
    """Merge Echo hooks into *settings*, preserving user hooks.
    ...
    for event_name, vc_entries in _ECHO_HOOKS.items():
        existing: list = hooks.get(event_name, [])
        if not isinstance(existing, list):
            logger.warning(
                "hooks.%s is not a list (%s) — overwriting with Echo hooks",
                event_name,
        ...
        for vc_entry in vc_entries:
            if not _list_contains_echo_entry(existing):
                existing.append(vc_entry)
                logger.debug("Added Echo hook entry for %s", event_name)
            else:
                logger.debug(
                    "Echo hook entry for %s already present — skipping",
                    event_name,
                )
...
def _remove_hooks(settings: dict) -> None:
    """Remove Echo hook entries from *settings*, preserving user hooks.
    ...
    for event_name, entries in hooks.items():
        ...
        filtered = [e for e in entries if not _entry_is_echo(e)]
...
def _entry_is_echo(entry: dict) -> bool:
    """Return ``True`` if *entry* was installed by Echo.
...
def _list_contains_echo_entry(entries: list) -> bool:
    """Return ``True`` if any entry in *entries* belongs to Echo."""
    return any(_entry_is_echo(e) for e in entries)
