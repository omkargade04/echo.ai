...
def mock_provider(monkeypatch):
    """Mock TTS provider — patched into tts_engine module."""
    mock = AsyncMock()
    mock.is_available = True
    mock.provider_name = "mock"
    mock.synthesize = AsyncMock(return_value=_PCM_BYTES)
    monkeypatch.setattr("echo.tts.tts_engine.create_tts_provider", lambda: mock)
    return mock
...
@pytest.fixture
def engine(mock_provider, mock_player, mock_livekit, narration_bus):
...
class TestLifecycle:
    ...
    async def test_start_starts_all_components(self, engine, mock_provider, mock_player, mock_livekit):
        await engine.start()
    ...
    async def test_stop_stops_all_components(self, engine, mock_provider, mock_player, mock_livekit):
        await engine.start()
        # Reset call counts from start()
        mock_provider.stop.reset_mock()
...
class TestCriticalRouting:
    ...
    async def test_critical_synthesizes_text(self, engine, narration_bus, mock_provider):
        await engine.start()
        narration = _make_narration("Permission needed!", NarrationPriority.CRITICAL)
        await narration_bus.emit(narration)
        await asyncio.sleep(0.05)
        mock_provider.synthesize.assert_awaited_with("Permission needed!")
        await engine.stop()
    ...
    async def test_critical_no_pcm_skips_playback(
        self, engine, narration_bus, mock_provider, mock_player, mock_livekit
    ):
...
class TestNormalRouting:
    ...
    async def test_normal_synthesizes(self, engine, narration_bus, mock_provider):
        await engine.start()
        narration = _make_narration("Reading file.", NarrationPriority.NORMAL)
        await narration_bus.emit(narration)
        await asyncio.sleep(0.05)
        mock_provider.synthesize.assert_awaited_with("Reading file.")
        await engine.stop()
    ...
    async def test_normal_no_pcm_skips(self, engine, narration_bus, mock_provider, mock_player, mock_livekit):
...
class TestLowRouting:
    ...
    async def test_low_over_threshold_skips(
        self, engine, narration_bus, mock_player, mock_provider
    ):
        ...
        await narration_bus.emit(narration)
        await asyncio.sleep(0.05)
        mock_provider.synthesize.assert_not_awaited()
        mock_player.enqueue.assert_not_awaited()
        await engine.stop()
...
class TestConsumeLoop:
    ...
    async def test_consume_loop_processes_event(self, engine, narration_bus, mock_provider):
        """An event emitted on the bus is picked up and processed."""
        await engine.start()
        narration = _make_narration("Hello.", NarrationPriority.NORMAL)
        await narration_bus.emit(narration)
        await asyncio.sleep(0.05)
        mock_provider.synthesize.assert_awaited_once_with("Hello.")
        await engine.stop()

    async def test_consume_loop_handles_exception(
        self, engine, narration_bus, mock_provider, mock_player
    ):
        ...
        assert mock_provider.synthesize.await_count == 2
    ...
    async def test_consume_loop_multiple_events(
        self, engine, narration_bus, mock_provider
    ):
        ...
        assert mock_provider.synthesize.await_count == 5
        await engine.stop()
...
class TestAlertManagerIntegration:
    ...
    async def test_constructor_with_event_bus_creates_alert_manager(
        self, mock_provider, mock_player, mock_livekit, narration_bus
    ):
    ...
    async def test_start_starts_alert_manager(
        self, mock_provider, mock_player, mock_livekit, narration_bus, monkeypatch
    ):
        event_bus = EventBus(maxsize=64)
        mock_am = AsyncMock()
    ...
    async def test_critical_without_alert_manager_still_works(
        self, engine, narration_bus, mock_player, mock_provider, mock_livekit
    ):
    ...
    async def test_repeat_callback_replays_alert_and_narration(
        ...
        mock_provider.synthesize.assert_awaited_once_with("Permission needed!")
