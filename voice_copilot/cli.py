"""Command-line interface for Voice Copilot.

Provides ``voice-copilot start``, ``stop``, ``status``, ``install-hooks``,
and ``uninstall`` commands.  The entry point is registered via
``pyproject.toml`` as ``voice-copilot = "voice_copilot.cli:cli"``.
"""
...
from voice_copilot.config import (
    DEFAULT_PORT,
    PID_FILE,
    VOICE_COPILOT_DIR,
    get_port,
...
_LOG_FILE = VOICE_COPILOT_DIR / "server.log"
...
def _setup_logging_to_file() -> None:
    ...
    """
    VOICE_COPILOT_DIR.mkdir(parents=True, exist_ok=True)
...
def _run_server(port: int) -> None:
    """Start uvicorn with the Voice Copilot FastAPI app.
...
@click.group()
def cli() -> None:
    """Voice Copilot -- Real-time audio bridge for AI coding agents."""
...
def start(port: int | None, daemon: bool, skip_hooks: bool) -> None:
    """Start the Voice Copilot server."""
    ...
    click.echo(f"Starting Voice Copilot on port {port}...")
...
@cli.command()
def stop() -> None:
    """Stop the background Voice Copilot server."""
    pid = _read_pid()
    ...
    click.echo(f"Stopping Voice Copilot server (PID {pid})...")
...
def status(port: int | None) -> None:
    """Show Voice Copilot server status."""
    port = _resolve_port(port)
    pid = _read_pid()
...
@cli.command()
def uninstall() -> None:
    """Remove Voice Copilot hooks and clean up.
    ...
    click.echo(click.style("Voice Copilot cleaned up.", fg="green"))
