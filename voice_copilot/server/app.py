"""FastAPI application factory for Voice Copilot.

Creates the FastAPI app with lifespan management for the EventBus,
TranscriptWatcher, and Summarizer.  The ``create_app()`` function is
the single entry point used by the CLI and ``uvicorn`` alike.
"""
...
from voice_copilot.interceptors.transcript_watcher import TranscriptWatcher
...
async def lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    stopped cleanly.
    """
    logger.info("Voice Copilot server starting up")
    ...
    try:
        ...
        logger.info("Voice Copilot server shutting down")
