"""Parse raw Claude Code hook JSON into VoiceCopilotEvent instances."""
...
def parse_hook_event(raw_json: dict) -> VoiceCopilotEvent | None:
    """Convert a raw Claude Code hook payload into a VoiceCopilotEvent.
    ...
    try:
        ...
        logger.exception(
            "Failed to construct VoiceCopilotEvent from hook payload "
            "(hook_event_name=%s)",
            hook_event_name,
        )
        return None
...
def _parse_post_tool_use(raw: dict, session_id: str) -> VoiceCopilotEvent:
...
def _parse_notification(raw: dict, session_id: str) -> VoiceCopilotEvent:
...
def _parse_stop(raw: dict, session_id: str) -> VoiceCopilotEvent:
...
def _parse_session_start(raw: dict, session_id: str) -> VoiceCopilotEvent:
...
def _parse_session_end(raw: dict, session_id: str) -> VoiceCopilotEvent:
